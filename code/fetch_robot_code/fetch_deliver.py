...
class MoveBaseClient(object):
    ...
    def goto(self, x, y, theta_z, theta_w, frame="map"):
        move_goal = MoveBaseGoal()
        move_goal.target_pose.pose.position.x = x
        move_goal.target_pose.pose.position.y = y
        move_goal.target_pose.pose.orientation.z = theta_z
        move_goal.target_pose.pose.orientation.w = theta_w
...
if __name__ == '__main__':
	...
	move_base_clinet = MoveBaseClient()

	# raising the torso
	rospy.loginfo("Raising torso ...")
	torso_client.move_to([0.0])
	...
	while not rospy.is_shutdown():
		...
		while not pick_res or not place_res:
			...
			rospy.loginfo("Start to deliver the object ...")
			grasping_client.tuck()
			grasping_client.clear_scene()

			rospy.loginfo("Descend torso...")
			torso_client.move_to([0.0])

			while not delivered:
				rospy.loginfo("Start to searching customer...")
				if count_deliver % 2 == 0:
					move_base_clinet.goto(2.873, -1.130, -0.247, 0.969)
				else:
					move_base_clinet.goto(1.685, -2.280, -0.672, 0.740)

				rospy.wait_for_service('recog_face')
				detected_people = rospy.ServiceProxy('recog_face', recog_face)
				recog_people = detected_people()

				if recog_people.target == target_people:
					rospy.loginfo("Target people is found...")
					grasping_client.deliver()
					grasping_client.open()
					grasping_client.tuck()
					delivered = True
			...
			move_base_clinet.goto(1.900, 1.059, -0.386, 0.923)
