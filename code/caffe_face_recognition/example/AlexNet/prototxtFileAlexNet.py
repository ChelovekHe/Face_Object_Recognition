...
def lenet(lmdb, batch_size, include_acc=False):
    ...
    n.conv1 = L.Convolution(n.data, kernel_size=11, num_output=20, stride=4, weight_filler=dict(type='gaussian', std=0.01),bias_filler=dict(type='constant', value=0))
    ...
    n.conv2 = L.Convolution(n.pool1, kernel_size=5, group=2, num_output=80, pad=2, weight_filler=dict(type='gaussian', std=0.01),bias_filler=dict(type='constant', value=0.1))
    ...
    n.conv4 = L.Convolution(n.conv3, kernel_size=3, group=2, num_output=160, pad=1, weight_filler=dict(type='gaussian', std=0.01),bias_filler=dict(type='constant', value=0))
    n.relu4 = L.ReLU(n.conv4, in_place=True)
    n.conv5 = L.Convolution(n.conv4, kernel_size=3, group=2, num_output=80, pad=1, weight_filler=dict(type='gaussian', std=0.01),bias_filler=dict(type='constant', value=0.1))
    n.relu5 = L.ReLU(n.conv5, in_place=True)
    n.pool5 = L.Pooling(n.conv5, kernel_size=3, stride=2, pool=P.Pooling.MAX)
    n.fc6   = L.InnerProduct(n.pool5, num_output=320, weight_filler=dict(type='gaussian', std=0.005),bias_filler=dict(type='constant', value=0.1))
    n.relu6 = L.ReLU(n.fc6, in_place=True)
    n.drop6 = L.Dropout(n.fc6, in_place=True)
    n.fc7   = L.InnerProduct(n.fc6, num_output=320, weight_filler=dict(type='gaussian', std=0.005),bias_filler=dict(type='constant', value=0.1))
...
with open('/home/xiao/Desktop/caffe_face/example/alexnet_auto_train.prototxt', 'w') as f:
    f.write(str(lenet(train_lmdb, 41, include_acc=False)))
    
with open('/home/xiao/Desktop/caffe_face/example/alexnet_auto_test.prototxt', 'w') as f:
